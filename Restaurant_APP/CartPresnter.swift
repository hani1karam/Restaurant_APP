...
class CartPresnter{
    private var view: CartPresnterToDos?
    weak var Home: HomeVC?
    public var FavProductitem = [AddToCartProduct]()
    private var toDoIndicator = Reguest()
    var loginModel:AddToCart?
    
    init(view: CartPresnterToDos) {
        self.view = view
    }
    
    func Fav(param:[String:Any]){
        toDoIndicator.AddCart(userInfoDict:param){ [weak self] (response, error) in
